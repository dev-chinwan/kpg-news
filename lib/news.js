...
function cloneLocalArticles() {
  const items = (newsData.articles || []).map((a) => normalizeArticle({ ...a }));
  return sortByLatestPosted(items);
}

async function cloneArticles() {
  const cloudinaryArticles = await getStoredArticles();
  if (cloudinaryArticles.length > 0) {
    const items = cloudinaryArticles.map((a) => normalizeArticle({ ...a }));
    await syncLocalNewsJsonBestEffort({ articles: items });
    return sortByLatestPosted(items);
  }
...
export async function getNewsBySlug(slug) {
  ...
  return all.find((a) => a.id === key || a.slug === key) || null;
}
